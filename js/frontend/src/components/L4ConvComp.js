...
const L4ConvPanel = ({ convsDataArray, singleMode, proType, filename }) => {
  ...
  const exportToPDF = (conversation, type) => {
    ...
    html2canvas(cap, { scale: scaleFactor }).then((canvas) => {
      ...
      doc.save(`${filename}_${type}_conversation_${conversation.conversationId + 1}.pdf`);
