...
const FilesAndAnalyzePage = ({ isValidUser, userData }) => {
  ...
  const columns = [
    ...
    },
    {
      field: 'analyzed', headerName: 'analyzed', headerAlign: 'center', align: 'center', flex: 1, renderCell: (params) => {
        return <span className={FilesPageStyle.files_grid} style={{borderRadius: '20px',width: "40%", fontSize: "20px",backgroundColor: params.value === 1 ? "#77DD76" : "#FF6962"}}> {params.value === 1 ? "Yes" : "No"} </span>
  ...
  if (isValidUser) {
    return (
      <div style={{ display: 'flex', height: '900%', marginTop: "-5px" }}>
        {
          isAnalyzeLoading &&
        ...
        <Box style={{ width: '100%' }} sx={{ mr: -1, color: 'black' }}>
          <Stack direction={'column'}>
            <Box sx={{ mt: "10px", mx: "10px", p: 1.5, height: "calc(4%)", background: 'linear-gradient(135deg, rgba(0, 0, 0, 0.06), rgba(255, 255, 255, 0))', backdropFilter: 'blur(25px)', WebkitBackdropFilter: 'blur(25px)', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.18)', boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.37)',
            ...
            <Box sx={{ mt: "6px", mx: "10px", p: 4,  background: 'linear-gradient(135deg, rgba(0, 0, 0, 0.06), rgba(255, 255, 255, 0))', backdropFilter: 'blur(25px)', WebkitBackdropFilter: 'blur(25px)', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.18)', boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.37)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <UploadComp className={FilesPageStyle.info_panel} userAnalyzeDataCallback={userAnalyzeDataCallback} />
            </Box>
            <Box id="files_panel_container" className={FilesPageStyle.files_grid} sx={{ mt: "6px", mx: "10px", height: "58.5vh", display: 'flex', flexDirection: 'column',  background: 'linear-gradient(135deg, rgba(0, 0, 0, 0.06), rgba(255, 255, 255, 0))', backdropFilter: 'blur(50px)', WebkitBackdropFilter: 'blur(25px)', borderRadius: '20px', border: '1px solid rgba(255, 255, 255, 0.18)', boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.37)', textAlign: 'center' }}>
